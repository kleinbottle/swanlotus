...
% 
\node [text=DarkSlateGray] at (-0.4, 0.25) {$x^2 + y^2 = 1$};
%
\node [text=DarkSlateGray] at (-0.4, -0.2) {$\cos\theta = \frac{x}{1} = x$};
\node [text=DarkSlateGray] at (-0.4, -0.4) {$\sin\theta = \frac{y}{1} = y$};
\node [text=DarkSlateGray] at (-0.4, -0.6) {$\tan\theta = \frac{\sin\theta}{\cos\theta} = \frac{y}{x}$};
%
\end{tikzpicture}
\end{document}

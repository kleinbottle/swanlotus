...
\item
  ...
  the \_imaginary unit, which is defined as \(i^2 = -1\). Since every
...
paid at the checkout counter while shopping---is made possible the
...
\$5\$5. It is common to write this as \(2 + 3 + 5 = 10\). The sum \(10\)
is correct but it is derived from the commutativity and associativity of
addition. In commutativity, we have \(2 + 3 = 3 + 2\), the order of the
\emph{two} operands does not matter. In associativity, we have three
operands, it does not matter which pair we add first:
\((2 + 3) + 5 = 2 + (3 + 5)\). Together, these two operations allow us
to be casual about how we order the numbers when adding many of them.
...
\hypertarget{why-is-division-by-aro-not-allowed}{%
\subsubsection{Why is division by aro not
allowed?}\label{why-is-division-by-aro-not-allowed}}

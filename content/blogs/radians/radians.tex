...
\emph{each term} to radians. This is a layer of bookkeeping we may
...
certain properties} may represented by an infinite sum of sinusoids.
...
\centering
\includegraphics[width=0.8\textwidth,height=\textheight]{images/squarefourier.png}
...
evident that the more terms we add, the better the match between the
original signal and its approximation, denoted generically by \(s_*(t)\)
in the graph..
...
sine waves which have no corners? The answer is that the series goes on
forever, ``infinity confers the equality''.
...
about without radians. With that epiphany on beauty, we shall conclude
our tale of two measures.

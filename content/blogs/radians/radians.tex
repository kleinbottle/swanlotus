...
in a similar unitless measure, and the radian fits the bill.
...
cycle of the square wave---shown below--- is repeated periodically
forever.
...
The successive partial sums of the RHS of \cref{eq:squarefourier} are
denoted as \(s_1(t)\), \(s_2(t)\), etc., denoting the first term, the
sum of the first and second term etc. It is evident from
\cref{fig:squarefourier} that the more terms we add, the better the
match between the original signal and its approximation, denoted
generically by \(s_*(t)\) in the graph.

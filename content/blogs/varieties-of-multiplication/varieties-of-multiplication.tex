...
\section{Multiplication as a binary
...
\section{Multiplication as repeated
...
\section{Square numbers}\label{square-numbers}}
...
\hypertarget{fractions-and-negative-numbers}{%
\section{Fractions and negative
numbers}\label{fractions-and-negative-numbers}}
...
of Academic Success}}. I urge you to read it if you feel the need. For
now, it will suffice if you can accept that \((-1)(-1) = +1 = 1\), or
better still, \href{https://math.stackexchange.com/a/384866/19098}{read
and understand this justification}.

\hypertarget{laws-of-arithmetic}{%
\section{Laws of arithmetic}\label{laws-of-arithmetic}}

We now return to the assertion, made at the start of this blog, that
multiplication is a \emph{binary} operation: something that happens with
\emph{two} mathematical objects. You might object that we can and do
multiply three numbers. For example, \(2 \times 3 \times 5 = 30\) and no
one would doubt the veracity of that assertion. Why then is
...
introducing logic and rigour into their subject. They wanted to
construct an intellectual edifice that was stable and durable.

Classifying all four arithmetic operations as binary and identifying the
laws they obeyed was one of these foundational reforms.

The numbers we encounter on a daily basis are called the
\href{https://en.wikipedia.org/wiki/Real_number}{real numbers}, although
this is not their mathematically rigorous definition. \emojifont {😉}
\normalfont

The multiplication of real numbers obeys certain laws, of which we list
three here:
...
\item
  ...
  \(a(bc) = (ab)c\).
\item
  \href{https://mathworld.wolfram.com/Distributive.html}{Distributivity
  of multiplication over addition}: \(a(b+c) = ab + ac\) and
  \((b+c)a = ba + ca\).
\end{enumerate}

Of the three, we have already spoken about commutativity before. Here,
we will concentrate on what associativity implies and why we need it.

\hypertarget{associativity-of-multiplication}{%
\section{Associativity of
...
\hypertarget{symbols-for-multiplication-across-time-and-need}{%
\section{Symbols for multiplication across time and
need}\label{symbols-for-multiplication-across-time-and-need}}

If you are sharp-eyed, you might have noticed how we previously denoted
the multiplication of two negative numbers by enclosing each of them in
parentheses: (). The same symbols were also used to define associativity
and distributivity above. We now look at the chequered history of how
the symbol for multiplication has changed with time and need.

\hypertarget{times-sign}{%
\subsection{Times sign}\label{times-sign}}

The symbol for multiplication that we first learn is the rotated cross
that looks like \(\times\). It is called the ``multiplication sign'' and
is usually read as ``times'', as we have already seen. It serves
reasonably well even when we outgrow the whole numbers and move onto
fractions.
...
\subsection{Juxtaposition without any
...
when handwritten. To avoid confusion, a convention was adopted that when
two \emph{algebraic variables,} representing unknown quantities, were
written next to each other or \emph{juxtaposed,} it indicated the
multiplication of the two quantities.

There is no intervening symbol between the two variables. Thus,
\(x \times y = (x)(y) = xy\). Note that this convention is for algebraic
variables only. We cannot use this convention with digits representing
numbers because of \emph{place value} in our decimal system. The number
\(45\) does \emph{not} represent the product of \(4\) and \(5\) but
actually means \(40 + 5 = 45\).

\hypertarget{centred-dot}{%
\subsection{Centred dot}\label{centred-dot}}

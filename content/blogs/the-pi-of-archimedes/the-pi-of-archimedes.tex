...
\(n\tan\frac{\pi}{n}\) as \(n\) increases.}\tabularnewline
\toprule\noalign{}
...
\item
  We calculate \(\sin 15°\) from \(\cos 30°\) using the half-angle
  formula: \[
  \begin{aligned}
...
of which is recommended.\textbackslash{}
...
\begin{enumerate}
\item
  Set \(n = 6\), \(\theta = \frac{\pi}{6}\), and \(k = 0\). Note that
  \(\sin\theta = \frac{1}{2}\), \(\cos\theta = \frac{\sqrt 3}{2}\), and
  \(\tan\theta = \frac{\sqrt 3}{3}\) from Pythagoras' Theorem.
\item
...
\end{equation*} \$\$ to compute the values of
\(\sin\frac{\theta}{2^k}\), \(\cos\frac{\theta}{2^k}\), and
\(\tan\frac{\theta}{2^k}\) from the value of
\(\cos\frac{\theta}{2^{k-1}}\).

\begin{enumerate}
\item
  If \(k = 4\) stop; otherwise repeat steps
  {[}\citeproc{ref-ref:increment}{\textbf{ref:increment?}}{]} to
  {[}\citeproc{ref-ref:compute}{\textbf{ref:compute?}}{]}.
\item
  Tabulate your results showing the values of \(k\),
  \(2^{k}n\sin\frac{\theta}{2^{k}}\),
  \(2^{k}n\tan\frac{\theta}{2^{k}}\).
\end{enumerate}

=++++++++++++++++++++++++++++++++++++++++=

To better understand what is happening, let us concentrate on the term
\(n\sin\theta\) and understand how the recursion works:
...
The recurrence relation linking \(\theta_{k}\) to \(\theta_{k-1}\) has
now been established. We tabulate below the values for \(k = 0\),
\(n = n_{0} = 6\), and
\(\theta = \theta_{0} = \frac{180°}{n_{0}} = \frac{180°}{6} = 30°\)
derived from the half-angle formulae:

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.0602}}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.2169}}@{}}
\caption{Polygon perimeter estimates using the half-angle formulae.
...
\(n=2^kn_{0}\)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\$\theta=\frac{\theta_{0}}{2^{k}}\$2\textsuperscript{\{k\}n\_\{0\}\left[\frac{1-\cos\left[\frac{\theta_{0}}{2^{k-1}}\right]}{2}\right]}\{\frac{1}{2}\}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\(n\)
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
\(n\tan\theta\)
